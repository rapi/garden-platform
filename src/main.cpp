...
// const char* ssid = "HUAWEI-aah9";
// const char* password = "4gd19aib";
int PIN_COUNT=15;
int TIME_PER_ZONE=20;
bool IS_STOP_WATERING=false;
bool IS_STARTED_WATERING=false;
int active_pin = 0;
int pins[] = {13,23,33,32,26,27,14,15,4,25,18,19,5,21,22};
...
void GET_INFO(AsyncWebServerRequest *request){
  ...
  for(int i=0;i<sizeof(pins)/sizeof(pins[0]);i++){
    response+="{\"pin\":"+String(pins[i])+",\"interval\":"+String(zone_time[i])+",\"isEnabled\":"+(digitalRead(pins[i])==LOW?"true":"false")+"}";
...
void handleRoot(AsyncWebServerRequest *request) {
  ...
  "<html>"
  "<body>"
  "<input placeholder='Interval' id='interval'>"
  "<button onclick='fetch(\"/set_interval/?interval=\"+document.getElementById(\"interval\").value)'>Save Interval</button><br/>"
  "<input placeholder='Interval' id='timer'>"
  "<button onclick='fetch(\"/set_timer/?timer=\"+document.getElementById(\"timer\").value)'>Save Time</button><br/>"
  "<button onclick='fetch(\"/start\")'>Start Watering</button>"
  "<button onclick='fetch(\"/stop\")'>Stop Watering</button>"
  "</body></html>");
}
...
void loop(void) {
  ...
  Serial.print(""+timeClient.getFormattedTime()+" Timer on: ");
